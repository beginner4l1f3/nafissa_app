...
export default function ChooseOurFutureClone() {
  ...
  const STEP_UI: Record<string, { title: string; desc: string; options: Choice[] }> = {
    ...
    flights: {
      ...
      desc:
        "Os aviões são menos eficientes do que os carros e, quanto mais e more longos forem os voos, maior será a energia necessária.",
      options: [
    ...
    <div className="min-h-screen w-full bg-gradient-to-b from-slate-50 to-white p-4 md:p-8">
      <header className="mx-auto flex max-w-6xl items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          {/* Substituímos o CF pelo logo */}
          <img
            src="/icons/museusdomar.jpeg"
            alt="Museus do Mar"
            className="h-10 w-10 rounded-2xl object-cover shadow"
          />
          <h1 className="text-xl font-bold md:text-2xl">
            Museus do Mar - Módulo de Desenvolvimento Cognitivo Sobre Poluição
          </h1>
        </div>
        <button
          ...
          onClick={() => {
            ...
            setRevealed({});
            setModalOpen(false);
            setSel({ ...DEFAULTS }); // limpar selecções
          }}
        >
          Recomeçar
        </button>
