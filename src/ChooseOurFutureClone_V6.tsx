...
const calculateTotalEmissions = (selections: Record<string, string>) => {
  ...
  Object.keys(selections).forEach(categoryKey => {
    ...
    const emissionCategory = CATEGORY_MAPPING[categoryKey as keyof typeof CATEGORY_MAPPING];
    
    if (selection && emissionCategory && EMISSION_DATA[emissionCategory][selection]) {
...
export default function ChooseOurFutureClone() {
  ...
  const pieData = useMemo(() => {
    ...
    return categories.map(categoryKey => {
      ...
      const emissionCategory = CATEGORY_MAPPING[categoryKey as keyof typeof CATEGORY_MAPPING];
      const value = selection && emissionCategory && EMISSION_DATA[emissionCategory][selection] 
                ...
                <div className="h-48 w-full">
                  <ResponsiveContainer>
                    <PieChart>
                      ...
                      <Tooltip formatter={(v: number) => `${v.toFixed(2)} tCO₂e/ano`} />
                      <Legend />
                    </PieChart>
                  </ResponsiveContainer>
                </div>
